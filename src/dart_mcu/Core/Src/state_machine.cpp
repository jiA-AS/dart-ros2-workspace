...
void FSM::update()
{
    ...
    if (xTaskGetTickCount() - ext_judge_last_receive_time >
            pdMS_TO_TICKS(1000) &&
    ...
    msgDartStatus.judge_online =
        !(xTaskGetTickCount() - ext_judge_last_receive_time >
          pdMS_TO_TICKS(1000));
...
class ActionWaitForAllMotorOnline : public OpenFSMAction
{
  public:
    ...
    {
        // 电机上电后默认状态为IDLE
        if (motor::MotorYawLS.motor_state_ !=
                motor::E_MotorState::DISCONNECTED &&
            motor::MotorLoad[0].motor_state_ !=
                motor::E_MotorState::DISCONNECTED &&
            motor::MotorLoad[1].motor_state_ !=
  ...
  public:
    ...
    void update(OpenFSM &fsm) const override
    {
        ...
        if (abs(motor_controller::MotorTriggerLSController
                ...
                6000000) < 10000 &&
            abs(motor_controller::MotorYawLSController
                    .current_angle_with_rounds_ -
                40000) < 1000)
        {
            fsm.nextAction();
                   ...
                   bool reset_when_miss_target = true)
{
    ...
    if (xTaskGetTickCount() - last_autoaim_update_tick > 33)
    { // 30 fps
        ...
        if (msgDartParams_.auto_aim_enabled)
        {
            ...
            else if (reset_when_miss_target)
            {
                ...
                {
                    motor_controller::MotorYawLSController
                        .target_angle_with_rounds_ = msgDartParams_.primary_yaw;
    ...
    void update(OpenFSM &fsm) const override
    {
        ...
        if (RC_Data.Switch_Left == RC_SW_UP || RC_Data.Switch_Left == RC_SW_MID)
        {
            ...
            // 扳机锁定在初始位置，不可触发操作，可以操作Yaw、Load电机和扳机丝杆
            {
                ...
                if (RC_Data.ch0 > 900 && RC_Data.ch0 < 1100)
                {
                    if (motor_controller::MotorYawLSController.state_ !=
                        motor_controller::
                            E_PID_Velocity_Angle_Controller_State::
                                ANGLE_CONTROL)
                    {
                        motor_controller::MotorYawLSController.set_state(
                            motor_controller::
                        ...
                        msgDartParams.last_param_update_time =
                            rmw_uros_epoch_millis();
                    }
                    // 从msgDartParams里获取
                    // 判断是否更新自瞄
                    if (!updateAutoAim(msgDartParams) &&
                        !msgDartParams.auto_aim_enabled)
                        motor_controller::MotorYawLSController
                            .target_angle_with_rounds_ =
                            msgDartParams.primary_yaw;
                }
                else
                {
                    ...
                    if (RC_Data.ch0 <= 700 && yaw_switch_state != Triggered)
                        ...
                        motor_controller::MotorYawLSController
                            .target_velocity_ = 100;
                    else if (yaw_switch_state == Triggered &&
                             RC_Data.ch0 <= 900)
                        motor_controller::MotorYawLSController
                            .target_velocity_ = 0; // 保证不小于零
                    motor_controller::AutoAimController.reset();
                }
            }
            ...
            int16_t base_velocity = 0;
            {
                switch (fsm.custom<Dart_FSM>()->ActionRemote_MotorLoad_State)
                {
                    case 0:
                        ...
                        {
                            fsm.custom<Dart_FSM>()
                                ->ActionRemote_MotorLoad_State = 2;
                        }
                        else if ((RC_Data.ch3 > 1400 && RC_Data.ch3 < 1600) |
                                 (RC_Data.ch3 >= 366 && RC_Data.ch3 < 950))
                        {
                            fsm.custom<Dart_FSM>()
                                ->ActionRemote_MotorLoad_State = 3;
                        }
                        else if (RC_Data.ch3 < 366)
                        {
                            fsm.custom<Dart_FSM>()
                                ->ActionRemote_MotorLoad_State = 1;
                        }
                        break;
                    case 1:
                        base_velocity =
                            CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD;
                        // 状态转移
                        if ((RC_Data.ch3 >= 1600) |
                            (motor_controller::MotorLoadController[0]
                                     .current_angle_with_rounds_ >=
                        ...
                        {
                            fsm.custom<Dart_FSM>()
                                ->ActionRemote_MotorLoad_State = 0;
                            base_velocity = 0;
                        }
                        break;
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        fsm.custom<Dart_FSM>()->ActionRemote_MotorLoad_State =
                            0;
                }
            }
            ...
            switch (fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reset_State)
            {
                case 0:
                    if (RC_Data.Switch_Left == RC_SW_UP &&
                        RC_Data.ch4_wheel <= 514)
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reset_State = 1;
                    }
                    break;
                case 1:
                    // 将拉簧拉到底
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reset_State = 2;
                ...
                case 2:
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reset_State = 3;
                    }
                    break;
                case 3:
                    // 装填电机向上运动到初始位置
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reset_State = 4;
                        setTriggerServotoReload();
                    }
                    break;
                case 4:
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reset_State = 0;
                    }
                    break;
                default:
                    fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reset_State =
                        0;
            }

            // 升降机装填控制
            switch (
                fsm.custom<Dart_FSM>()->ActionRemoteandReload_Slidedown_State)
            {
                case 0:
                    if (RC_Data.Switch_Left == RC_SW_MID &&
                        RC_Data.ch4_wheel <= 514)
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Slidedown_State = 1;
                ...
                case 1:
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Slidedown_State = 2;
                        setSlidedownServotoCut();
                    }
                    break;
                case 2:
                    ...
                    if (RC_Data.ch4_wheel > 514)
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Slidedown_State = 0;
                    break;
                default:
                    fsm.custom<Dart_FSM>()
                        ->ActionRemoteandReload_Slidedown_State = 0;
            ...
            switch (fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reload_State)
            {
                case 0:
                    if (RC_Data.ch4_wheel >= 1622 &&
                        RC_Data.Switch_Left == RC_SW_MID)
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reload_State = 1;
                    }
                    break;
                case 1:
                    // 装填电机向下运动到装填位置
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reload_State = 2;
                        fsm.custom<Dart_FSM>()->ActionGeneral_Timer0_ =
                            xTaskGetTickCount();
                        setLoadServotoDOWN();
                        setTriggerServotoTrigger();
                ...
                case 2:
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reload_State = 3;
                    }
                    break;
                case 3:
                    base_velocity =
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reload_State = 4;
                        setLoadServotoUP();
                        setTriggerServotoReload();
                    }
                    break;
                case 4:
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandReload_Reload_State = 0;
                    }
            }

            motor_controller::MotorLoadController[0].target_velocity_ =
                base_velocity +
                motor_controller::MotorLoadSyncController.output;
        ...
        else if (RC_Data.Switch_Left == RC_SW_DOWN)
        {
            ...
            if (!fsm.custom<Dart_FSM>()->launch_operating_)
            {
                ...
                if (launch_grant_)
                {
                    ...
                    soundEffectManager.addSoundEffect(
                        BUZZER_NOTE(buzzer_approach));
                }
            }
            else
            {
                if (!fsm.custom<Dart_FSM>()
                         ->ActionRemoteandMatch_launch_complete_)
                {
                    if (motor_controller::MotorLoadController[0]
                            .current_angle_with_rounds_ <=
                        CONFIG_MOTOR_LOAD_ANGLE_UP)
                    {
                        base_velocity =
                            -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 4;
                    }
                    else
                    {
                        base_velocity =
                    ...
                    {
                        fsm.custom<Dart_FSM>()
                            ->ActionRemoteandMatch_launch_complete_ = true;
                        fsm.custom<Dart_FSM>()->ActionGeneral_Timer1_ =
                            xTaskGetTickCount();
                        base_velocity = 0;
                        setTriggerServotoTrigger();
                ...
                else
                {
                        ...
                        pdMS_TO_TICKS(CONFIG_LAUNCH_WAIT_TIME))
                    {
                        ...
                        {
                            fsm.custom<Dart_FSM>()
                                ->ActionRemoteandMatch_launch_complete_ = false;
                            fsm.custom<Dart_FSM>()->launch_operating_ = false;
                            // 切闭环
                            motor_controller::MotorLoadController[0].set_state(
                                motor_controller::
...
class ActionMatch_Enter : public OpenFSMAction
{
    ...
    void update(OpenFSM &fsm) const override
    {
        ...
        motor_controller::MotorTriggerLSController.target_angle_with_rounds_ =
            ...
            msgDartProtocols.primary_force_offset;
        motor_controller::MotorYawLSController.target_angle_with_rounds_ =
...
class ActionMatch_Wait : public OpenFSMAction
{
    void enter(OpenFSM &fsm) const override
    {
            ...
            game_progress == 4)
        {
            if (msgDartStatus.dart_launch_process >= 2)
                fsm.custom<Dart_FSM>()->ActionMatch_Wait_Continuous_Fire =
                    false;
    ...
    void update(OpenFSM &fsm) const override
    {
            ...
            game_progress == 3)
        {
            ...
            dart_mcu_log(
                "Match Wait:Ready stage ended, Update primary_yaw to %d",
                msgDartProtocols.primary_yaw);
            // 重置自瞄控制器
            motor_controller::AutoAimController.reset();
            msgDartStatus.primary_yaw_offset = 0;
        ...
        if (RC_Data.Switch_Left != RC_SW_UP)
        {
            ...
            launch_grant_ |= (dart_remaining_time > 2 &&
                              dart_remaining_time <= 20 && game_progress == 4);
        ...
        if (((((ext_game_status.stage_remain_time < 4 ||
                dart_remaining_time < 3) &&
               game_progress == 4) ||
...
class ActionMatch_Launch : public OpenFSMAction
{
    ...
    void update(OpenFSM &fsm) const override
    {
        ...
        switch (fsm.custom<Dart_FSM>()->ActionMatch_Launch_State)
        {
                ...
                if (msgDartProtocols.auto_aim_enabled)
                {
                    ...
                    else
                    {
                        ...
                        motor_controller::MotorYawLSController
                            ...
                            msgDartProtocols.primary_yaw +
                            msgDartStatus.primary_yaw_offset +
                            msgDartProtocols.auxiliary_yaw_offsets
                                [msgDartStatus.dart_launch_process];
            ...
            case 1:
                // 等待电机稳定
                if (abs(motor_controller::MotorYawLSController
                            .target_angle_with_rounds_ -
                        motor_controller::MotorYawLSController
            ...
            case 3:
                ...
                {
                    base_velocity =
                        -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 5;
                }
                else
                {
                    base_velocity =
                        ...
                        -CONFIG_MOTOR_LOAD_ANGLE_LAUNCH_UP)
                {
                    ...
                    motor_controller::MotorLoadController[0].set_state(
                        motor_controller::
                            E_PID_Velocity_Angle_Controller_State::OPEN_LOOP);
            ...
            case 4:
                    ...
                    pdMS_TO_TICKS(CONFIG_LAUNCH_WAIT_TIME))
                {
                    ...
                    else
                    {
                        ...
                        dart_mcu_log(
                            "Expect to launch but stuck: last: %" PRIu64,
                            last_launch_time_);
                    }
                }
                break;
...
class ActionMatch_NewReload : public OpenFSMAction
{
    /*新的装填机制:发射->滑台到安装位置前方->大摆锤旋转摇臂到安装角度->总线舵机工作，宣旋转大摆锤pitch到安装角度->滑台移动主动安装镖体->吸盘气阀打开->滑台往后->大摆锤旋转到允许发射角度->发射*/
    // 待施工
};
        ...
        switch (fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reload_State)
        {
            case 0:
                ...
                if (xTaskGetTickCount() -
                        fsm.custom<Dart_FSM>()->ActionGeneral_Timer3_ >
                    pdMS_TO_TICKS(CONFIG_LAUNCH_WAIT_TIME))
                    fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reload_State =
                        1;
                break;
            case 1:
                // 装填电机向下运动到装填位置
...
void Dart_FSM::start()
{
    ...
    // 注册新的动作
    OpenFSM::RegisterAction<ActionMatch_Reload>("ActionMatch_New_Reload");
    ...
    OpenFSM::RegisterState("StateRemote", {"ActionRemote"},
                           E_Dart_State::Remote);
    // 修改为new_reload
